...
class Buffer extends AbstractMonitor
{
  ...
  public Buffer( int p_tamanio_buffer )
  {
    tamanio_buffer = p_tamanio_buffer ;
    buffer = new double[tamanio_buffer] ;
  }
  public void depositar( double valor )
  {
    enter();
    while( cont == tamanio_buffer ){
      produce.await();
    }
    primera_libre = (primera_libre + 1) % tamanio_buffer;
  ...
  public double extraer()
  {
    ...
    primera_ocupada = (primera_ocupada + 1) % tamanio_buffer;
    cont--;
    produce.signal();
    leave();
    return buffer[primera_ocupada];
  }
}
